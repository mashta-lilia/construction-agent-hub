...
class ConfigFactory:
    """Base for settings sections: reads each annotated field from the
    environment under `__prefix__`, casting it to the annotated type.

    Not an ABC — it declares no abstract members, so inheriting from `abc.ABC`
    would only claim an enforcement that does not exist.
    """
    ...
    def _cast_value(self, value: str, var_type: type):
        if var_type is str:
            ...
            return tuple(value.split(",")) if var_type is tuple[str] else value.split(",")
        elif var_type in (list[bool], tuple[bool]):
            return (
                tuple(map(lambda x: x.lower() in ("true", "1", "yes"), value.split(",")))
                if var_type is tuple[bool]
                else list(map(lambda x: x.lower() in ("true", "1", "yes"), value.split(",")))
            )
