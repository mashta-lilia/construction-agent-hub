...
- `mail/` — тільки build-скелет на базі `docker-mailserver`, без логіки
  ...
  контейнера з `MAIL_SEED_ACCOUNT`/`MAIL_SEED_PASSWORD` (обов'язкові
  env-змінні) і відмовляється стартувати, якщо їх не задано. Без жодного
  акаунта Dovecot сам відмовляється стартувати і контейнер зациклюється на
  рестарті кожні ~2 хв. Це не робочий поштовий ящик, лише спосіб тримати
  контейнер живим до задачі з реальним провіжином акаунтів.
