...
- `mail/` — тільки build-скелет на базі `docker-mailserver`, без логіки
  провіжину поштових акаунтів по проєктах. `mail/seed-account.sh` генерує
  один акаунт при старті контейнера з `MAIL_SEED_ACCOUNT`/
  `MAIL_SEED_PASSWORD` (обов'язкові env-змінні, `docker-compose.yml`) — без
  жодного акаунта Dovecot відмовляється стартувати і контейнер
