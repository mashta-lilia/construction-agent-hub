...
> **Статус: чернетка, ще не реалізовано на фронті.** Повний порт фронтенда
> (S2-FE-01, PR #5) прибрав тодішній `src/services/apiClient.ts` разом з рештою
> foundation-скелета і заново відновив лише сам клієнт
> (`frontend/src/services/apiClient.ts`) — без жодного реального виклику,
> бо backend-ендпоінтів ще немає. `npm run api:types` і
> `frontend/src/services/api-schema.d.ts` нижче — це ЦІЛЬ, а не поточний стан:
> генерація типів з OpenAPI-схеми підключається в тому PR, що вперше реально
> викликає ці ендпоінти (див. `docs/pr-plan.md`), а не раніше — генерувати
> типи зі схеми, якої ще нема, безглуздо. До того моменту не орієнтуватись
> на шляхи/поля нижче як на вже наявні у коді.
...
`npm run api:types` генерує `frontend/src/services/api-schema.d.ts` (усі шляхи в
цьому документі — від кореня репозиторію). Цей файл — чернетка
