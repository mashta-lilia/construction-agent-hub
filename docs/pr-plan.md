...
> замість пошагового порту.** Порт прототипу виявився не таким, що ділиться
> на самодостатні PR без постійного ре-рев'ю "тимчасово непрото типу
> компонент" — тому зроблено одним внеском, з явним застереженням про розмір
...
> `develop`:
> - `frontend/src/services/apiClient.ts` відновлено (порожній клієнт, без
>   реальних викликів) — підключення до справжніх ендпоінтів і генерація
>   типів (`npm run api:types`) відбувається в тому PR, що вперше додає
>   робочий backend-виклик, не раніше (див. `docs/api-contract.md`).
